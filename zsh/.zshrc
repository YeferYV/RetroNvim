...
[[ -e $APPDATA                                    ]] && export HOME="/c/Users/$USERNAME"
[[ -e $APPDATA && -e $CONDA_PREFIX                ]] && export CONDA_PREFIX=$(/bin/cygpath --unix $CONDA_PREFIX) # fixes `command not found` on d: drive
...
export CONDA_PREFIX=${CONDA_PREFIX:-$HOME/.pixi/envs/wezterm}
export EDITOR="nvim"
...
export RETRONVIM_BIN=$(printf $HOME/.*/extensions/yeferyv.retronvim*/bin) 2>/dev/null # printf outputs the first match only
...
export WEZTERM_CONFIG_FILE="$CONDA_PREFIX/opt/retronvim/zsh/wezterm.lua"
...
export PATH="$PATH:$CONDA_PREFIX/opt/zsh-patina/bin"
export PATH="$PATH:$RETRONVIM_BIN"
export PATH="$PATH:$ZEROBREW_PREFIX/bin"

[[ "$OSTYPE" == "linux-gnu" && ! -e $FONT_LINUX   && $TERM_PROGRAM != "WezTerm" ]] && firacode-nerdfont-installer     2>/dev/null
[[ "$OSTYPE" == "darwin"    && ! -e $FONT_MACOS   && $TERM_PROGRAM != "WezTerm" ]] && firacode-nerdfont-installer     2>/dev/null
[[ "$OSTYPE" == "cygwin"    && ! -e $FONT_WINDOWS && $TERM_PROGRAM != "WezTerm" ]] && firacode-nerdfont-installer.cmd 2>/dev/null
